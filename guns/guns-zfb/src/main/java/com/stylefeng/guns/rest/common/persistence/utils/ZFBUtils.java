...
import trade.model.builder.*;
import trade.model.hb.*;
...
public class ZFBUtils {
    private static Log log = LogFactory.getLog(ZFBUtils.class);
...
//                            + (long) (Math.random() * 10000000L);
        String outTradeNo = "tradepay" + 0101;

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店消费”
        String subject = "Mtime影城门店当面付消费";
...
//                            + (long) (Math.random() * 10000000L);
        String outTradeNo = orderId + "MtimeCinema";

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "Mtime影城当面付扫码消费";
...
//        String totalAmount = "0.01";
        ...
        // 业务扩展参数，目前可添加由支付宝分配的系统商编号(通过setSysServiceProviderId方法)，详情请咨询支付宝技术支持
        ExtendParams extendParams = new ExtendParams();
        extendParams.setSysServiceProviderId("2016101500696037");
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                String filePath = String.format("E:\\Git\\project3_microservice\\guns\\guns-zfb\\src\\main\\resources" + "/qr-%s.png",
